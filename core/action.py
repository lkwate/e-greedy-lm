...
def main(model_name: str, output_file: str, k: int = 10, factor: int = 10, name: str = "name"):
    embedding = AutoModel.from_pretrained(model_name).embeddings.word_embeddings.weight
    tokenizer = AutoTokenizer.from_pretrained(model_name)
    create_action(embedding, tokenizer, output_file, k=k, factor=factor, name=(name == "name"))
